...
async def app_command_setup_preflight(interaction: discord.Interaction) -> bool:
    ...
    # Discord.py may leave Namespace._values as None for slash commands
    # that have no options (for example /send-broadcast-panel and
    # /broadcast-bots). Treat that the same as an empty mapping so the
    # global preflight check does not crash before the command runs.
    values = (getattr(namespace, "_values", None) or {}) if namespace else {}
