...
DATABASE_FILE = os.path.join(DATA_DIR, "guilds.sqlite3")
...
def _load_legacy_json() -> dict:
    """يقرأ guilds.json القديم مرة واحدة حتى لا تضيع إعدادات السيرفرات."""
...
def _ensure_database():
    """ينشئ قاعدة التخزين ويهاجر JSON القديم تلقائياً عند أول تشغيل."""
    connection = _connect()
    try:
        connection.execute(
            """
        ...
        row = connection.execute(
            "SELECT schema_version, data_json FROM app_state WHERE id = 1"
        ).fetchone()
        if row is None:
            legacy_data = _load_legacy_json()
            connection.execute(
                "INSERT INTO app_state (id, schema_version, data_json) VALUES (1, ?, ?)",
                (SCHEMA_VERSION, json.dumps(legacy_data, ensure_ascii=False)),
            )
            # نترك نسخة JSON القديمة كما هي كنسخة أمان، لكن كل التعديلات الجديدة
            # ستذهب إلى SQLite ولن تعيد الإعدادات إلى الصفر بعد Restart.
        elif row[0] < SCHEMA_VERSION:
            connection.execute(
                "UPDATE app_state SET schema_version = ? WHERE id = 1",
                (SCHEMA_VERSION,),
            )
        connection.commit()
    finally:
    ...
    for path in _list_backups():
        try:
            ...
            if isinstance(data, dict):
                # النسخة سليمة - نرجعها إلى SQLite حتى لا نضل نعتمد على
                # النسخة الاحتياطية وحدها.
                _write(data)
                return data
        except (json.JSONDecodeError, OSError):
...
def _backup_now():
    """تأخذ نسخة JSON احتياطية من قاعدة SQLite الحالية."""
    os.makedirs(BACKUP_DIR, exist_ok=True)
    try:
        data = _read()
...
def _read() -> dict:
    ...
    try:
        with _connect() as connection:
            row = connection.execute(
                "SELECT data_json FROM app_state WHERE id = 1"
            ).fetchone()
        if row is None:
            return {}
        ...
        if restored:
            return restored
        # لا نبدأ من صفر بصمت إذا كانت القاعدة تالفة؛ ننشئ قاعدة فارغة فقط
        # إذا لم توجد أي نسخة احتياطية صالحة.
        _write({})
        return {}


def _write(data: dict):
    # SQLite تعمل داخل Transaction، لذلك لا يعيد Restart الإعدادات إلى الصفر
    # ولا يترك JSON ناقصاً إذا توقف البوت أثناء الحفظ.
    _ensure_files()
...
def _default_guild() -> dict:
    ...
    return {
        ...
        "clear_v2": {
            "allowed_role_ids": [],       # مين يقدر يستخدم "مسح @شخص عدد"
            "log_channel_id": None,
            "max_amount": 50,             # أقصى عدد رسائل يقدر يمسحها بالمرة
            "protected_role_id": None,    # مين رتبته بنفس موقع هاي الرتبة أو أعلى، رسائله محمية
        },
        "prison": {
        ...
        "security": {
            ...
            "permission_escalation": {
                "allowed_role_ids": [],   # رتب مستثناة (يقدروا يضيفوا صلاحيات خطيرة بدون عقاب)
                "log_channel_id": None,
                "notify_role_id": None,
            },
            "guild_update": {
                "allowed_role_ids": [],   # رتب مستثناة (يقدروا يغيروا إعدادات السيرفر بدون عقاب)
                "log_channel_id": None,
                "notify_role_id": None,
            },
            "rate_limit": {
                "allowed_role_ids": [],   # رتب مستثناة تماماً من هاد الحد (متل الأونر)
                "log_channel_id": None,
                "notify_role_id": None,
                "jail_role_id": None,
                "threshold_count": 5,     # كم إجراء خطير يفعّل الحماية
                "threshold_seconds": 15,  # خلال كم ثانية
            },
        },
        "xp": {
            "xp_min": 5,
        ...
        "xp_period": {
            "last_daily_reset_date": None,   # آخر تاريخ (YYYY-MM-DD بتوقيت فلسطين) صار فيه تصفير يومي
            "last_weekly_reset_date": None,  # آخر تاريخ سبت (YYYY-MM-DD بتوقيت فلسطين) صار فيه تصفير أسبوعي
        },
        "message_log": {
            "channel_id": None,   # لوق الرسائل المحذوفة/المعدلة
        },
        "join_log": {
            "channel_id": None,   # لوق دخول أعضاء جدد
        },
        "leave_log": {
            "channel_id": None,   # لوق مغادرة أعضاء
        },
        "command_log": {
            "channel_id": None,   # لوق كل أوامر البوت (نصية وslash)
        },
        "quest_staff": {
            "allowed_role_ids": [],
            "log_channel_id": None,
            ...
            "quest_channel_id": None,   # روم نشر المهام
            "target_role_id": None,    # الرتبة يلي موجهة إلها المهمة (بتنمنشن عند النشر)
        },
        "quest_highstaff": {
            "allowed_role_ids": [],
            "log_channel_id": None,
        ...
        "mentions": {
            "log": {},   # {user_id: [{"by": mentioner_id, "channel_id": ..., "message_id": ..., "at": iso}, ...]}
        },
    }
...
class Storage:
    ...
    async def update_security(guild_id: int, subsection: str, updates: dict):
        """subsection: 'bot_add' | 'prune' | 'webhook' | 'channels' | 'roles' | 'permission_escalation' | 'guild_update' | 'rate_limit'"""
        async with _lock:
            data = _read()
            gid = str(guild_id)
            if gid not in data:
    ...
    @staticmethod
    async def get_broadcast_campaigns(status: str = None) -> list:
        """ترجع حملات البرودكاست المحفوظة حتى يمكن استئنافها بعد Restart."""
        async with _lock:
            data = _read()
            result = []
            for gid, guild_data in data.items():
                campaign = guild_data.get("broadcast", {}).get("campaign")
                if not campaign:
                    continue
                if status is not None and campaign.get("status") != status:
                    continue
                result.append((int(gid), dict(campaign)))
            return result
    ...
    async def reset_period_counters_if_needed(guild_id: int):
        """
        بتصفر عدّادات اليوم/الأسبوع (xp_day / xp_week) لكل المستخدمين، لبكتي xp وvoice_xp.
        يومي: كل يوم الساعة 1:00 ليلاً بتوقيت فلسطين.
        أسبوعي: كل سبت الساعة 12:00 ظهراً بتوقيت فلسطين.
        """
        from datetime import datetime, timezone
        from utils.time_parsing import PALESTINE_TZ

        now_local = datetime.now(PALESTINE_TZ or timezone.utc)
        ...
        async with _lock:
            ...
            if gid not in data:
                data[gid] = _default_guild()
            data[gid].setdefault("xp_period", {"last_daily_reset_date": None, "last_weekly_reset_date": None})
            period = data[gid]["xp_period"]

            reset_day = now_local.hour >= 1 and period.get("last_daily_reset_date") != today_str
            # السبت = weekday() == 5 (الاثنين=0 ... الأحد=6)
            reset_week = (
                now_local.weekday() == 5 and now_local.hour >= 12
                and period.get("last_weekly_reset_date") != today_str
            )
            ...
            if reset_day:
                period["last_daily_reset_date"] = today_str
            if reset_week:
                period["last_weekly_reset_date"] = today_str
    ...
    # ---------- تتبّع المنشنات ("مين منشنني بآخر 24 ساعة") ----------

    @staticmethod
    async def add_mention(guild_id: int, target_user_id: int, by_user_id: int, channel_id: int, message_id: int):
        from datetime import datetime, timezone
        async with _lock:
            data = _read()
            gid = str(guild_id)
            if gid not in data:
                data[gid] = _default_guild()
            data[gid].setdefault("mentions", {"log": {}})
            data[gid]["mentions"].setdefault("log", {})
            uid = str(target_user_id)
            entries = data[gid]["mentions"]["log"].setdefault(uid, [])
            entries.append({
                "by": by_user_id,
            ...
            # حماية بسيطة: ما نخلي لستة شخص وحد تكبر بلا حدود (سبام منشنات مثلاً)
            del entries[:-100]
            _write(data)

    @staticmethod
    async def get_recent_mentions(guild_id: int, target_user_id: int, within_hours: int = 24) -> list:
        """بترجع منشنات الشخص بآخر within_hours ساعة بس (الأقدم بتنشال تلقائياً)، من الأحدث للأقدم."""
        from datetime import datetime, timezone, timedelta
        async with _lock:
            data = _read()
            gid = str(guild_id)
            uid = str(target_user_id)
            entries = data.get(gid, {}).get("mentions", {}).get("log", {}).get(uid, [])
            ...
            # نحدّث المخزّن بالقائمة المنقّاة (نشيل يلي أقدم من الحد نهائياً)
            if len(fresh) != len(entries):
                data[gid]["mentions"]["log"][uid] = fresh
                _write(data)

            return list(reversed(fresh))

    @staticmethod
    async def prune_all_old_mentions(older_than_hours: int = 24):
        """تنظيف دوري عام (كل السيرفرات وكل الأشخاص) - يشيل أي منشن أقدم من الحد حتى لو محدا فحص قائمته."""
        from datetime import datetime, timezone, timedelta
        async with _lock:
            data = _read()
            ...
            changed = False
            for gid, gdata in data.items():
                log = gdata.get("mentions", {}).get("log", {})
                for uid, entries in list(log.items()):
                    fresh = []
                    for e in entries:
                        try:
                            at = datetime.fromisoformat(e["at"])
