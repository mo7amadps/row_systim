...
# أوامر $ف و $ق فيهم استثناء: ما بلزم تكتب $ قبلهم (تقدر تكتب "ف" أو "ق" مباشرة)
NO_PREFIX_COMMANDS = ("ف", "ق")


def get_prefix(bot_: commands.Bot, message: discord.Message):
    prefixes = ["$"]
    first_word = message.content.split(" ")[0].split("\n")[0] if message.content else ""
    if first_word in NO_PREFIX_COMMANDS:
        prefixes.append("")
