...
class BroadcastPanelView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        """لوحة البرودكاست قديمة وموجودة في قناة عامة، لذلك لا نعتمد على مكانها فقط."""
        if not interaction.guild or not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(
                "هذه اللوحة مخصصة لأعضاء الإدارة فقط.",
                ephemeral=True,
            )
            return False
        return True
...
class BroadcastSystem(commands.Cog):
    ...
    async def _require_admin(self, interaction: discord.Interaction) -> bool:
        """تحقق دفاعي داخل كل مسار حساس، بما فيها إرسال المودال مباشرة."""
        if (
            not interaction.guild
            ...
            if interaction.response.is_done():
                await interaction.followup.send(
                    "هذه الميزة مخصصة لأعضاء الإدارة فقط.",
                    ephemeral=True,
                )
            else:
                await interaction.response.send_message(
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not message.author.guild_permissions.administrator:
            await message.reply("ليس لديك الصلاحيات اللازمة لاستخدام هذا الأمر.")
            return
