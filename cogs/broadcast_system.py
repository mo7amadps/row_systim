...
class BroadcastSystem(commands.Cog):
    ...
    async def whitelist_borad(
        ...
        """
        تغيير سياسة الوصول محصور بمالك السيرفر فقط.

        هذا يمنع أي Administrator آخر من إعطاء نفسه صلاحية البرودكاست،
        ...
        if interaction.user.id != guild.owner_id:
            await interaction.response.send_message(
                "هذا الأمر مخصص لمالك السيرفر فقط، حتى لو كنت Administrator.",
                ephemeral=True,
            )
            return
