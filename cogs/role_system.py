...
from utils.checks import has_any_role, collect_roles, assignable_roles, bot_missing_permissions, setup_permission_check
...
class RoleEditSession:
    def __init__(self, cog: "RoleSystem", ctx: commands.Context, target: discord.Member):
        ...
        all_assignable = assignable_roles(ctx.author, ctx.guild, include_own_top=(target.id == ctx.author.id))
...
class FinalConfirmView(discord.ui.View):
    ...
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        r = conf["role"]
        if not has_any_role(fresh_invoker, r["allowed_role_ids"]) and fresh_invoker.id != guild.owner_id:
        ...
        still_assignable_ids = {
            role.id for role in assignable_roles(fresh_invoker, guild, include_own_top=(target.id == fresh_invoker.id))
        }
        ...
        try:
            ...
            # مثلاً حدى الرتب انشالت من السيرفر أثناء ما المستخدم كان مستني بالقائمة
            await interaction.response.edit_message(
                content="❌ صار خطأ من ديسكورد وأنا عم أعدل الرتب (ممكن حدى الرتب انشالت). جرب مرة ثانية.",
                embed=None, view=None,
            )
            return
...
class RoleSystem(commands.Cog):
    ...
    @app_commands.describe(
        allowed_role_1="أول رتبة مسموحلها تستخدم أمر تعديل الرتب",
    ...
    @commands.check(setup_permission_check("role"))
    @commands.guild_only()
    async def role_cmd(self, ctx: commands.Context, member: discord.Member = None):
        ...
        # فيك تفتح القائمة على أي شخص حتى لو رتبته أعلى منك - بس assignable_roles()
        # بتتكفل إنه ما تقدر تلمس/تختار غير الرتب يلي تحت رتبتك أنت (شوف RoleEditSession)
        try:
            session = RoleEditSession(self, ctx, member)
            await session.start()
        except discord.HTTPException:
            await ctx.reply("❌ صار خطأ من ديسكورد وأنا عم أفتح قائمة الرتب. جرب مرة ثانية.")
