...
class PrisonSystem(commands.Cog):
    ...
    async def _perform_release(self, invoker: discord.Member, guild: discord.Guild, member: discord.Member, reply):
        ...
        jail_role = guild.get_role(p["jail_role_id"])
        ...
        try:
            if jail_role and jail_role in member.roles:
                await member.remove_roles(jail_role, reason=f"فك السجن بواسطة {invoker}")
            if to_restore:
                await member.add_roles(*to_restore, reason=f"فك السجن بواسطة {invoker}")
        except discord.Forbidden:
    ...
    async def jail_cmd(self, ctx: commands.Context, member: discord.Member = None, *, reason: str = None):
        ...
        try:
            if roles_to_remove:
                await member.remove_roles(*roles_to_remove, reason=f"سجن بواسطة {ctx.author}")
            await member.add_roles(jail_role, reason=f"سجن بواسطة {ctx.author}")
        except discord.Forbidden:
