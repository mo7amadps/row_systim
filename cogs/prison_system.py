...
from utils.checks import has_any_role, collect_roles, can_target, bot_missing_permissions, setup_permission_check
...
class PrisonSystem(commands.Cog):
    ...
    @commands.check(setup_permission_check("prison", role_key="imprison_role_ids"))
    @commands.guild_only()
    ...
    @commands.check(setup_permission_check("prison", role_key="release_role_ids"))
    @commands.guild_only()
    ...
    async def on_message(self, message: discord.Message):
        ...
        # نفس فكرة setup_permission_check بالضبط: ما منرد إطلاقاً على حدا ما معه
        # صلاحية مُعدة بـ set-up-prison، حتى قبل ما نشوف إذا كتب الأمر صح أو لأ.
        conf = await Storage.get_guild(message.guild.id)
        release_role_ids = conf["prison"].get("release_role_ids", [])
        if not has_any_role(message.author, release_role_ids):
            return
