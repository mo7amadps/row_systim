...
class RankSystem(commands.Cog):
    ...
    @commands.check(setup_permission_check("add_role"))
    @commands.guild_only()
    ...
    async def promote_cmd(self, ctx: commands.Context, member: discord.Member = None, amount: int = None):
        ...
        try:
            ...
            # مثلاً رتبة انشالت من السيرفر أثناء التنفيذ - منرجع رسالة واضحة بدل ما البوت يطلع "خطأ غير متوقع"
            await ctx.reply("❌ صار خطأ من ديسكورد وأنا عم أرقي الشخص (ممكن حدى الرتب انشالت). جرب مرة ثانية.")
            return

        # بدون منشن للشخص وبدون منشن للرتبة - بس اسمها نص عادي
        await ctx.reply(
            f"✅ تم تغيير رتبة {member.display_name} لـ رتبة {new_rank} - {new_role.name}",
    ...
    @commands.check(setup_permission_check("remove_role"))
    @commands.guild_only()
    ...
    async def demote_cmd(self, ctx: commands.Context, member: discord.Member = None, amount: int = None):
        ...
        remaining_roles = [role for role in member.roles if role.id not in roles_to_remove_ids]

        try:
            if roles_to_remove:
                # PATCH واحد بدل طلب منفصل لكل رتبة، لذلك التخفيض أسرع بكثير.
                await member.edit(roles=remaining_roles, reason=f"تخفيض بواسطة {ctx.author}")
        except discord.Forbidden:
            await ctx.reply("❌ ما قدرت أخفض الشخص، تأكد إنه رتبة البوت أعلى من هاي الرتب.")
            return
        except discord.HTTPException:
            await ctx.reply("❌ صار خطأ من ديسكورد وأنا عم أخفض الشخص (ممكن حدى الرتب انشالت). جرب مرة ثانية.")
            return

        result_text = f"رتبة {new_rank} - {new_role.name}" if new_role else "بدون رتبة"
        # بدون منشن للشخص وبدون منشن للرتبة - بس اسمها نص عادي
        await ctx.reply(
            f"✅ تم تغيير رتبة {member.display_name} لـ {result_text}",
            allowed_mentions=discord.AllowedMentions.none(),
        )
