...
class RankSystem(commands.Cog):
    ...
    async def promote_cmd(self, ctx: commands.Context, member: discord.Member = None, amount: int = None):
        ...
        # allowed_mentions=none حتى ما يوصل إشعار منشن للشخص ولا لكل حاملي الرتبة الجديدة
        await ctx.reply(
            f"✅ تم ترقية {member.mention} من رتبة {current_rank} إلى رتبة {new_rank} ({new_role.mention})",
    ...
    async def demote_cmd(self, ctx: commands.Context, member: discord.Member = None, amount: int = None):
        ...
        # allowed_mentions=none حتى ما يوصل إشعار منشن للشخص ولا لكل حاملي الرتبة
        await ctx.reply(
            f"✅ تم تخفيض {member.mention} من رتبة {current_rank} إلى {result_text}",
            allowed_mentions=discord.AllowedMentions.none(),
        )
