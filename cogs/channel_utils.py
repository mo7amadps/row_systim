...
class ChannelUtils(commands.Cog):
    ...
    @app_commands.command(name="set-up-lock", description="إعداد أمر قفل القناة ($ف)")
    ...
    async def set_up_lock(
        ...
        await self._run_setup(interaction, "lock", "نظام $ف", "$ف",
    ...
    @app_commands.command(name="set-up-unlock", description="إعداد أمر فتح القناة ($ق)")
    @app_commands.describe(
    ...
    async def set_up_unlock(
        ...
        await self._run_setup(interaction, "unlock", "نظام $ق", "$ق",
                               allowed_role_1, log_channel, allowed_role_2, allowed_role_3, allowed_role_4)
    ...
    async def clear_cmd(self, ctx: commands.Context, amount: int = None):
        ...
        confirm = await ctx.send(f"🧹 تم مسح {len(deleted) - 1} رسالة.")
