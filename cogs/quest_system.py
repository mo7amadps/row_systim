...
class QuestSystem(commands.Cog):
    ...
    async def _run_quest_setup(
        ...
        embed.add_field(name="📢 روم نشر المهام", value=quest_channel.mention, inline=True)
        embed.add_field(name="🎯 المهمة موجهة لرتبة", value=target_role.mention, inline=True)
        embed.add_field(
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"كل مهمة بتنشر بروم {quest_channel.mention}، وبتتمنشن فيها رتبة {target_role.mention} تلقائياً.\n"
    ...
    @app_commands.describe(
        ...
        quest_channel="روم نشر المهام",
        target_role="الرتبة يلي موجهة إلها المهمة (بتتمنشن تلقائياً عند النشر)",
    ...
    async def set_up_quest_staff(
        ...
        quest_channel: discord.TextChannel, target_role: discord.Role,
    ...
    async def _start_quest_flow(self, ctx: commands.Context, tier: str):
        ...
        cfg = conf[section]
        if (
            not cfg["allowed_role_ids"] or not cfg["first_role_id"] or not cfg["last_role_id"]
            or not cfg["log_channel_id"] or not cfg["quest_channel_id"] or not cfg["target_role_id"]
        ):
            await ctx.reply(f"❌ نظام مهام {label} ما تم إعداده لسا (أو ناقص إعداد). استخدم `/set-up-quest-{tier}` أول.")
            return
        ...
        quest_channel = ctx.guild.get_channel(cfg["quest_channel_id"])
        target_role = ctx.guild.get_role(cfg["target_role_id"])
        if not quest_channel or not target_role:
            await ctx.reply(f"❌ روم نشر المهام أو الرتبة الموجهة إلها انشالت من السيرفر. أعد الإعداد بـ `/set-up-quest-{tier}`.")
            return
    ...
    async def finalize_quest(self, ctx: commands.Context, tier: str, quest_type: str, start_time_text: str, xp_text: str, interaction: discord.Interaction):
        ...
        if not quest_channel:
            await interaction.response.send_message(
                f"❌ روم نشر المهام المُعد انشال من السيرفر. أعد الإعداد بـ `/set-up-quest-{tier}`.", ephemeral=True
            )
            return
        ...
        quest_data = {
            ...
            "target_role_id": cfg["target_role_id"],
        ...
        if is_immediate:
            await interaction.response.send_message(f"✅ جاري نشر مهمة {label} الآن بروم {quest_channel.mention}...", ephemeral=True)
            ...
            await interaction.response.send_message(
                f"✅ تم جدولة مهمة {label} — رح تُنشر بروم {quest_channel.mention} الساعة **{local_str}** "
                "(توقيت فلسطين) وتنتهي بعدها بـ24 ساعة.",
                ephemeral=True,
            )
    ...
    async def _publish_quest(self, guild: discord.Guild, tier: str, quest_data: dict):
        ...
        # أول شي بالرسالة: كمية الـXP والمدة (24 ساعة)
        embed.description = (
            f"🎯 **{quest_data['goal']} XP**\n"
            f"⏰ **المدة: 24 ساعة** (تنتهي {discord.utils.format_dt(ends_at, style='R')})\n\n"
            "مين ما وصل الهدف بيتكرم تلقائياً!"
        )
        ...
        # آخر شي: رسالة قصيرة تحت الإيمبد تعمل منشن فعلي (بينج) لرتبة الهدف
        # (منشن جوا حقول الإيمبد ما بيعمل بينج فعلي على ديسكورد، فلازم يكون برسالة عادية)
        target_role = guild.get_role(quest_data.get("target_role_id"))
        if target_role:
            try:
                await channel.send(
                    content=f"📢 {target_role.mention} في مهمة {label} جديدة بانتظاركم!",
                    allowed_mentions=discord.AllowedMentions(roles=[target_role]),
                )
            except discord.Forbidden:
                pass
                    ...
                    if msg.embeds:
                        ...
                        if embed.description:
                            embed.description = re.sub(
                                r"🎯 \*\*\d+ XP\*\*", f"🎯 **{new_goal} XP**", embed.description, count=1
                            )
    ...
    async def on_xp_awarded(self, guild: discord.Guild, member: discord.Member, amount: int, bucket: str):
        ...
        for tier in TIERS:
            ...
            if ladder:
                top_role = guild.get_role(ladder[-1])
                if top_role and any(r.position > top_role.position for r in member.roles):
                    continue  # هو أصلاً برتبة أعلى من كل سلّم الترقية - تجاهل تماماً، كأنه ما سوى شي
    ...
    async def _handle_completion(self, guild: discord.Guild, tier: str, member: discord.Member, result: dict):
        ...
        if quest["type"] == "ترقية":
            ladder = quest.get("rank_ladder_role_ids") or []
            top_role = guild.get_role(ladder[-1]) if ladder else None
            if top_role and any(r.position > top_role.position for r in member.roles):
                return  # هو أصلاً برتبة أعلى من كل سلّم الترقية - ما منسوي ولا نمنشن شي
