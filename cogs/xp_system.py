...
from utils.xp_logic import is_valid_for_xp, roll_message_xp
...
class XPSystem(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if is_valid_for_xp(message.content, len(message.attachments)):
            amount = roll_message_xp(cfg["xp_min"], cfg["xp_max"])
        else:
            amount = 0
    ...
    @app_commands.describe(
        xp_min="أقل XP ممكن ياخده الشخص من رسالة حقيقية (رقم عشوائي بين هاد والأقصى)",
    ...
    async def set_up_xp(
        ...
        embed.add_field(name="XP لكل رسالة", value=f"{xp_min} - {xp_max} (عشوائي)", inline=True)
