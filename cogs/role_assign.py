...
from utils.checks import has_role, bot_missing_permissions, is_dangerous_role
...
class RoleAssignSystem(commands.Cog):
    ...
    @app_commands.describe(allowed_role="مين مسموحله يستخدم $رول", log_channel="قناة اللوق")
    @app_commands.guild_only()
    @app_commands.checks.has_permissions(administrator=True)
    async def set_up_roles(self, interaction: discord.Interaction, allowed_role: discord.Role, log_channel: discord.TextChannel):
        ...
        embed.add_field(name="مين مسموح يستخدمه", value=allowed_role.mention)
    ...
    async def role_cmd(self, ctx: commands.Context, member: discord.Member = None, *, role_query: str = None):
        ...
        await ctx.reply(f"{action} رتبة {role.mention} {'من' if 'شيل' in action else 'لـ'} {member.mention}")
