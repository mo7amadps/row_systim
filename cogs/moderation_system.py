...
class ModerationSystem(commands.Cog):
    ...
    async def _check_permission(self, ctx: commands.Context, section: str, setup_cmd: str) -> bool:
        ...
        s = conf[section]
        if not s["allowed_role_id"]:
            await ctx.reply(f"❌ النظام ما تم إعداده لسا. استخدم `/{setup_cmd}` أول.")
            return False
        if not has_role(ctx.author, s["allowed_role_id"]) and ctx.author.id != ctx.guild.owner_id:
    ...
    async def _run_setup(interaction: discord.Interaction, section: str, title: str, command_label: str,
                          allowed_role: discord.Role, log_channel: discord.TextChannel):
    ...
    @app_commands.describe(allowed_role="مين مسموحله يستخدم $rar", log_channel="قناة اللوق")
    @app_commands.guild_only()
    @app_commands.checks.has_permissions(administrator=True)
    async def set_up_rar(self, interaction: discord.Interaction, allowed_role: discord.Role, log_channel: discord.TextChannel):
        await self._run_setup(interaction, "rar", "نظام $rar", "$rar", allowed_role, log_channel)
    ...
    @app_commands.describe(allowed_role="مين مسموحله يستخدم $نك", log_channel="قناة اللوق")
    @app_commands.guild_only()
    @app_commands.checks.has_permissions(administrator=True)
    async def set_up_nickname(self, interaction: discord.Interaction, allowed_role: discord.Role, log_channel: discord.TextChannel):
        await self._run_setup(interaction, "nickname", "نظام $نك", "$نك", allowed_role, log_channel)
    ...
    @app_commands.describe(allowed_role="مين مسموحله يستخدم $تحذير", log_channel="قناة اللوق")
    @app_commands.guild_only()
    @app_commands.checks.has_permissions(administrator=True)
    async def set_up_warn(self, interaction: discord.Interaction, allowed_role: discord.Role, log_channel: discord.TextChannel):
        await self._run_setup(interaction, "warn", "نظام $تحذير", "$تحذير", allowed_role, log_channel)
    ...
    @app_commands.describe(allowed_role="مين مسموحله يستخدم $شيل", log_channel="قناة اللوق")
    @app_commands.guild_only()
    @app_commands.checks.has_permissions(administrator=True)
    async def set_up_unwarn(self, interaction: discord.Interaction, allowed_role: discord.Role, log_channel: discord.TextChannel):
        await self._run_setup(interaction, "unwarn", "نظام $شيل", "$شيل", allowed_role, log_channel)
