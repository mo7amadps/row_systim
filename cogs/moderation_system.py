...
class ModerationSystem(commands.Cog):
    ...
    @commands.check(setup_permission_check("nickname"))
    @commands.guild_only()
    ...
    async def nick_cmd(self, ctx: commands.Context, member: discord.Member = None, *, nickname: str = None):
        ...
        # فيك تغيّر نكك أنت لحالك بدون أي مشكلة، وفيك تغيّر نك حد أوطى منك،
        # بس ما فيك تغيّر نك حد رتبته أعلى منك أو تساويك (إلا إذا كنت الأونر)
    ...
    @commands.command(name="تحذير", aliases=["ت"])
    @commands.check(setup_permission_check("warn"))
    @commands.guild_only()
    @commands.cooldown(1, 3, commands.BucketType.user)
    ...
    @commands.check(setup_permission_check("unwarn"))
    @commands.guild_only()
    ...
    @commands.check(setup_permission_check("rar"))
    @commands.guild_only()
